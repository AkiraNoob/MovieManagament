...
const useGetWatchingList = (
  ...
  const queryReturn = useInfiniteQuery({
    ...config,
    queryKey: [QUERY_KEY.RECENT_LIST, perPage],
    queryFn: (ctx: QueryFunctionContext<ExtendsQueryKey, number>) =>
      apiGetWatchingMovies(generatePaginateFromInfiniteQuery(ctx, 1)),
    getNextPageParam(
...
export default useGetWatchingList;
